...
| 拍照录入 | 拍照/相册选图 → OCR Markdown 识别 → 批改结果 → 订正 | **后端 API** |
...
如需修改后端地址，编辑 `js/api.js` 第一行即可。拍照识别会由网关转发给独立 OCR 服务。
...
5. **Mock 数据 vs 真实数据**：拍照录入会调用真实后端与 OCR 服务，并展示 OCR Markdown、识别引擎和低置信度提示；其余页面按各自接口实现情况使用真实数据或 Mock 数据。
