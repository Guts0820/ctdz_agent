...
SERVICE_URLS = {
    "analysis": "http://localhost:8081",
    "error_analysis": "http://localhost:8082",
    "knowledge": "http://localhost:8083",
    "teaching": "http://localhost:8084",
    "state": "http://localhost:8085"
}
...
def submit_homework(request: SubmitRequest):
    try:
        ...
        if analysis_result["judge_result"] == "correct":
            state_result = call_state_service(request.student_id, "G-N-2-005", True, analysis_result["confidence"])
            return SubmitResponse(
                status="success",
                data={
        ...
        knowledge_result = call_knowledge_service(error_analysis_result)
        
        state_before = call_state_service(request.student_id, error_analysis_result["knowledge_id"], False, error_analysis_result["total_confidence"])
        
        frequency_result = call_frequency_check(request.student_id, error_analysis_result["knowledge_id"])
        ...
        state_after = call_state_service(request.student_id, error_analysis_result["knowledge_id"], False, error_analysis_result["total_confidence"])
        
        if state_after["should_generate_review"]:
            review_result = call_generate_review(request.student_id, error_analysis_result["knowledge_id"], state_after["knowledge_mastery_id"], state_after["master_level"])
        else:
            review_result = None
